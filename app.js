...
async function exportUpdatedPackage(){
  ...
  try{
    ...
    for(const file of state.packageFiles){const path=packageFilePath(file);if(!path)continue;if(file===jmsOriginal||/^jmsdata(?:\s.*|\(\d+\))?\.(xls|xlsx)$/i.test(file.name||''))continue;if(replacementNames.has(String(file.name||'').toUpperCase()))continue;zip.file(path,file)}
...
function activatePanel(panelId,tabButton=null){
  ...
  if(panelId==='invoice'){renderCustomerSummary();renderRecallResults()}if(panelId==='preview')renderPreview();if(panelId==='stockSearch')renderStockSearch();requestAnimationFrame(updateBackToTopButton)
}

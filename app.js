...
function resetExhibitionSequences(){
  ...
  const extra=hasDraft?'\n\n目前畫面有未完成資料；新展覽會會清空 Recall 文件記錄，但不會清除目前貨品或客戶草稿。':'';
  if(!confirm(`開始新展覽會「${exhibitionName}」？\n會建立全新的 Universe Records，舊展覽 Recall 記錄不會帶入；Invoice / Consignment / Quotation 下一號重設為 ${yy}0001。\n\nINV${yy}0001 / CON${yy}0001 / QUO${yy}0001${extra}`))return;
  state.exhibitionSession=newExhibitionId();try{localStorage.setItem(EXHIBITION_SESSION_KEY,state.exhibitionSession)}catch{}saveExhibitionName(exhibitionName);
  state.documentStore=emptyDocumentStore();state.recordCounters={invoice:1,consignment:1,quotation:1};state.recordsLoaded=true;state.recordsDirty=true;state.recordsFileName='Universe Records.json';state.dataMeta.records={name:UNIVERSE_RECORDS_FILE,lastModified:0,count:0,exhibitionId:state.exhibitionSession,exhibitionName,newTemplate:true};const recordsBtn=$('#exportUniverseRecordsBtn');if(recordsBtn)recordsBtn.disabled=false;
  try{localStorage.removeItem(DOCUMENT_STORE_KEY)}catch{}
  $('#invoiceNo').value=formatDocumentNo(1,yy,state.documentType);schedulePreview();if(hasDraft)scheduleDraftSave();else{clearDocumentDraft();markDraftBaseline()};renderRecallResults();updateDataVersionPanel();runHealthCheck();
  status('#addMessage',`新展覽會「${exhibitionName}」已建立全新文件記錄；下一號：INV${yy}0001 / CON${yy}0001 / QUO${yy}0001。請把新的 Universe Records.json 保存到本次展覽資料包。`,'ok');
}
...
function documentLabels(type=state.documentType){if(type==='consignment')return{type:'consignment',title:'Sales Consign',short:'Consignment',date:'Date',no:'Consign No.',items:'Consignment 貨品',confirm:'Confirm Consignment 並更新 jmsdata / Records',export:'匯出 Excel Consignment'};if(type==='quotation')return{type:'quotation',title:'Quotation',short:'Quotation',date:'Date',no:'Quotation No.',items:'Quotation 貨品',confirm:'Confirm Quotation 並更新 Records（不扣庫存）',export:'匯出 Excel Quotation'};return{type:'invoice',title:'Sales Invoice',short:'Invoice',date:'Date',no:'Invoice No.',items:'Invoice 貨品',confirm:'Confirm Invoice 並更新 jmsdata / Records',export:'匯出 Excel Invoice'}}
...
const UNIVERSE_RECORDS_FILE='Universe Records.json',UNIVERSE_RECORDS_SCHEMA_VERSION=1;
...
function universeRecordsJson(){return JSON.stringify(universeRecordsPayload(),null,2)}
function applyUniverseRecords(raw,{fallbackName='',fileName=UNIVERSE_RECORDS_FILE,filePath=''}={}){if(!raw||raw.app!=='Universe Invoice'||raw.recordType!=='Universe Records'||Number(raw.schemaVersion)!==UNIVERSE_RECORDS_SCHEMA_VERSION)throw new Error('Universe Records.json 格式不正確或版本不支援。');state.documentStore=normalizeDocumentStore(raw.documentStore);state.recordCounters=normalizeRecordCounters(raw.nextSequence);state.exhibitionSession=norm(raw.exhibitionId)||newExhibitionId();state.recordsFileName=fileName||UNIVERSE_RECORDS_FILE;state.recordsFilePath=filePath||'';state.recordsLoaded=true;state.recordsDirty=!norm(raw.exhibitionId)||!norm(raw.exhibitionName);try{localStorage.removeItem(DOCUMENT_STORE_KEY);localStorage.setItem(EXHIBITION_SESSION_KEY,state.exhibitionSession)}catch{}saveExhibitionName(norm(raw.exhibitionName)||norm(fallbackName)||state.packageName||'Exhibition');syncDocumentSequencesFromStore();renderRecallResults();updateDataVersionPanel();runHealthCheck();return{documents:officialDocumentCount(),exhibitionId:state.exhibitionSession,exhibitionName:state.exhibitionName,newTemplate:!norm(raw.exhibitionId)}}
async function importUniverseRecordsFile(file,{fallbackName='',filePath=''}={}){let raw;try{raw=JSON.parse(await file.text())}catch{throw new Error('Universe Records.json 不是有效 JSON。')}const info=applyUniverseRecords(raw,{fallbackName,fileName:file.name||UNIVERSE_RECORDS_FILE,filePath});state.dataMeta.records={name:file.name||UNIVERSE_RECORDS_FILE,lastModified:Number(file.lastModified)||0,count:info.documents,exhibitionId:info.exhibitionId,exhibitionName:info.exhibitionName,newTemplate:info.newTemplate};const btn=$('#exportUniverseRecordsBtn');if(btn)btn.disabled=false;updateDataVersionPanel();return info}
function exportUniverseRecords(){try{if(!state.recordsLoaded)throw new Error('尚未匯入或建立 Universe Records。');downloadBlob(new Blob([universeRecordsJson()],{type:'application/json;charset=utf-8'}),UNIVERSE_RECORDS_FILE);state.recordsDirty=false;if(state.dataMeta.records){state.dataMeta.records.count=officialDocumentCount();state.dataMeta.records.exhibitionId=state.exhibitionSession;state.dataMeta.records.exhibitionName=state.exhibitionName}const el=$('#jmsdataExportStatus');if(el){el.textContent=`已輸出 Universe Records.json：${officialDocumentCount()} 筆文件記錄；請把它取代回目前展覽資料包。`;el.className='notice ok'}return true}catch(err){console.error(err);alert('匯出 Universe Records.json 失敗：'+(err.message||err));return false}}
...
function exportJmsdata(){try{const wb=buildJmsdataWorkbook();XLSX.writeFile(wb,'jmsdata.xlsx',{bookType:'xlsx'});state.imageOverrideDirtyLots=new Set();state.imageOverrideDirty=0;updateImageOverrideStatus();const el=$('#jmsdataExportStatus');if(el){el.textContent=`已輸出 jmsdata.xlsx${state.imageOverrides.size?`，包括 ${state.imageOverrides.size} 項 LOTNO 圖片選擇`:''}；文件歷史另存於 Universe Records.json。請在「檔案」App 儲存時確認取代舊檔。`;el.className='notice ok'}return true}catch(err){console.error(err);alert('匯出 jmsdata 失敗：'+(err.message||err));return false}}
...
async function exportUpdatedPackage(){
  ...
  try{
    const zip=new JSZip(),jmsBuffer=XLSX.write(buildJmsdataWorkbook(),{bookType:'xlsx',type:'array'}),jmsOriginal=(state.packageFiles||[]).find(f=>/^jmsdata(?:\s.*|\(\d+\))?\.xlsx$/i.test(f.name)||/^jmsdata\.xlsx$/i.test(f.name)),jmsOriginalPath=jmsOriginal?packageFilePath(jmsOriginal):'',jmsDir=jmsOriginalPath.includes('/')?jmsOriginalPath.slice(0,jmsOriginalPath.lastIndexOf('/')):(state.packageName||'Exhibition Package'),jmsPath=`${jmsDir}/jmsdata.xlsx`,recordsOriginal=(state.packageFiles||[]).find(f=>/^universe\s*records\.json$/i.test(f.name||'')),recordsOriginalPath=recordsOriginal?packageFilePath(recordsOriginal):'',recordsDir=recordsOriginalPath.includes('/')?recordsOriginalPath.slice(0,recordsOriginalPath.lastIndexOf('/')):jmsDir,recordsPath=`${recordsDir}/${UNIVERSE_RECORDS_FILE}`;
    ...
    for(const file of state.packageFiles){const path=packageFilePath(file);if(!path)continue;if(file===jmsOriginal||file===recordsOriginal||/^jmsdata(?:\s.*|\(\d+\))?\.(xls|xlsx)$/i.test(file.name||'')||/^universe\s*records\.json$/i.test(file.name||''))continue;if(replacementNames.has(String(file.name||'').toUpperCase()))continue;zip.file(path,file)}
    zip.file(jmsPath,jmsBuffer);zip.file(recordsPath,universeRecordsJson());
    ...
    downloadBlob(blob,`${state.packageName||'Exhibition Package'}_Updated.zip`);state.recordsDirty=false;state.imageOverrideDirtyLots=new Set();state.imageOverrideDirty=0;state.packageImageDirtyFiles=new Set();updateImageOverrideStatus();
...
function clearCurrentDocument({keepCustomer=true}={}){releaseCustomImages();state.items=[];state.deliveryReturns=new Set();state.editingItemId=null;$('#discountAmount').value=0;$('#remark').value='';if(!keepCustomer){$('#customerCode').value='';$('#customerName').value='';$('#customerAddress').value='';$('#salesRate').value='';$('#customerTerms').value=''}renderItems();renderCustomerSummary();schedulePreview()}
...
async function importStockFile(f){
  ...
  if(!fullMap.size)throw new Error('找不到有效 LOTNO / ARTNO / PRICE');state.stockCatalog=fullMap;state.importConflicts=[];const imageInfo=importImageOverridesFromWorkbook(wb);rebuildInventoryMaps();state.dataMeta.stock={name:f.name||'',lastModified:Number(f.lastModified)||0,count:fullMap.size,available:state.products.size,duplicates:state.stockDuplicateLots.length,integrity:state.stockIntegrityIssues.length,documentConflicts:0,imageOverrideConflicts:imageInfo?.conflicts||0};for(const id of ['#exportJmsdataBtn','#exportImageOverridesBtn','#exportUpdatedPackageBtn']){const exportBtn=$(id);if(exportBtn)exportBtn.disabled=false}renderRecallResults();updateDataVersionPanel();runHealthCheck();return fullMap.size;
}
...
function updateDataVersionPanel(){
  ...
  rows.push(['Universe Records',m.records?`${m.records.name} · ${m.records.count} 筆文件 · ${m.records.exhibitionId||'未建立 ID'}`:'未載入']);
...
function healthSummaryRows(){
  ...
  if(!m.records)rows.push({level:'error',text:'尚未載入 Universe Records.json；Recall / 文件流水號沒有正式資料來源。'});else rows.push({level:'ok',text:`Universe Records：${m.records.count} 筆文件 · ${state.exhibitionName||m.records.exhibitionName||'未命名展覽'}`});
...
$('#exhibitionPackageInput').onchange=async e=>{
  ...
  state.documentStore=emptyDocumentStore();state.recordCounters={invoice:1,consignment:1,quotation:1};state.recordsLoaded=false;state.recordsDirty=false;state.recall=null;delete state.dataMeta.records;setRecallLock(false);renderRecallResults();
  const excels=files.filter(f=>/\.(xlsx?|xls)$/i.test(f.name)),jsonFiles=files.filter(f=>/\.json$/i.test(f.name));
  const byName=(re)=>excels.find(f=>re.test(f.name));
  let records=jsonFiles.find(f=>/^universe\s*records\.json$/i.test(f.name||''));
  ...
  try{
    if(!records)errors.push('找不到 Universe Records.json');else{const r=await importUniverseRecordsFile(records,{fallbackName:root,filePath:packageFilePath(records)});lines.push(`✓ ${records.name} · Exhibition ${r.exhibitionName} · 文件記錄 ${r.documents} 筆${r.newTemplate?' · 全新展覽記錄':''}`)}
    if(!stock)errors.push('找不到 jmsdata.xlsx');else{const n=await importStockFile(stock);lines.push(`✓ ${stock.name} · ${n} 件貨品 · Available ${state.products.size} · 圖片選擇 ${state.imageOverrides.size} 項 · ${new Date(stock.lastModified).toLocaleString('zh-HK')}`)}
    ...
    status('#stockStatus',stock?`已由資料包匯入 ${stock.name}：Available ${state.products.size} 件；圖片選擇 ${state.imageOverrides.size} 項。文件歷史來自 Universe Records.json。`:'尚未匯入倉存。',stock?'ok':'error');
...
$('#clearInvoiceBtn').onclick=()=>{const blocked=deliveredRecallItemsNeedingReturn();if(blocked.length)return alert(`目前有 ${blocked.length} 款貨品在 jmsdata 已是 Sold - Delivered。\n請先逐款按「已交貨 ✓」改成「未交貨」，確認貨品已退回後，才可清空／取消 Invoice。`);if(confirm(`清空目前 ${documentLabels().short} 貨品？`)){releaseCustomImages();state.items=[];renderItems();if(state.recall)scheduleDraftSave();else{state.deliveryReturns=new Set();clearDocumentDraft();markDraftBaseline()}}};

...
const UNIVERSE_RECORDS_FILE='Universe Records.json',UNIVERSE_RECORDS_SHEET='Universe Records',UNIVERSE_RECORDS_SCHEMA_VERSION=1,UNIVERSE_RECORDS_CHUNK_SIZE=30000;
...
function updateImageOverrideStatus(){
  ...
  const exportPackage=$('#exportUpdatedPackageBtn');
...
function emptyUniverseRecordsForWorkbook(fallbackName='',fileName='jmsdata.xlsx'){state.documentStore=emptyDocumentStore();state.recordCounters={invoice:1,consignment:1,quotation:1};state.exhibitionSession=newExhibitionId();state.recordsFileName=`${fileName||'jmsdata.xlsx'} / ${UNIVERSE_RECORDS_SHEET}`;state.recordsFilePath='';state.recordsLoaded=true;state.recordsDirty=true;try{localStorage.removeItem(DOCUMENT_STORE_KEY);localStorage.setItem(EXHIBITION_SESSION_KEY,state.exhibitionSession)}catch{}saveExhibitionName(norm(fallbackName)||state.packageName||'Exhibition');syncDocumentSequencesFromStore();renderRecallResults();updateDataVersionPanel();return{documents:0,exhibitionId:state.exhibitionSession,exhibitionName:state.exhibitionName,newTemplate:true}}
...
async function importLegacyUniverseRecordsFile(file,{fallbackName=''}={}){let raw;try{raw=JSON.parse(await file.text())}catch{throw new Error('舊 Universe Records.json 不是有效 JSON。')}const info=applyUniverseRecords(raw,{fallbackName,fileName:'jmsdata.xlsx'});state.dataMeta.records={name:`jmsdata.xlsx / ${UNIVERSE_RECORDS_SHEET}（由舊 JSON 遷移）`,lastModified:Number(file.lastModified)||0,count:info.documents,exhibitionId:info.exhibitionId,exhibitionName:info.exhibitionName,newTemplate:false,embedded:false,migratedFromJson:true};state.recordsDirty=true;updateDataVersionPanel();return info}
...
async function exportUpdatedPackage(){
  ...
  try{
    ...
    for(const file of state.packageFiles){const path=packageFilePath(file);if(!path)continue;if(file===jmsOriginal||/^jmsdata(?:\s.*|\(\d+\))?\.(xls|xlsx)$/i.test(file.name||'')||/^universe\s*records\.json$/i.test(file.name||''))continue;if(replacementNames.has(String(file.name||'').toUpperCase()))continue;zip.file(path,file)}
...
async function importStockFile(f){
  ...
  if(!fullMap.size)throw new Error('找不到有效 LOTNO / ARTNO / PRICE');state.stockCatalog=fullMap;state.importConflicts=[];const imageInfo=importImageOverridesFromWorkbook(wb);const recordsInfo=importUniverseRecordsFromWorkbook(wb,{fallbackName:state.packageName||state.exhibitionName||'',fileName:f.name||'jmsdata.xlsx',lastModified:f.lastModified});rebuildInventoryMaps();state.dataMeta.stock={name:f.name||'',lastModified:Number(f.lastModified)||0,count:fullMap.size,available:state.products.size,duplicates:state.stockDuplicateLots.length,integrity:state.stockIntegrityIssues.length,documentConflicts:0,imageOverrideConflicts:imageInfo?.conflicts||0,recordsEmbedded:!!state.dataMeta.records?.embedded};const exportBtn=$('#exportJmsdataBtn');if(exportBtn)exportBtn.disabled=false;const packageBtn=$('#exportUpdatedPackageBtn');if(packageBtn)packageBtn.disabled=false;renderRecallResults();updateDataVersionPanel();runHealthCheck();return fullMap.size;
}
...
function healthSummaryRows(){
  ...
  if(!m.records)rows.push({level:'error',text:'jmsdata.xlsx 尚未建立 Universe Records 工作表；Recall / 文件流水號沒有正式資料來源。'});else rows.push({level:'ok',text:`jmsdata Sheet 2「Universe Records」：${m.records.count} 筆文件 · ${state.exhibitionName||m.records.exhibitionName||'未命名展覽'}${m.records.newTemplate?' · 新展覽空白記錄':''}`});
...
$('#exhibitionPackageInput').onchange=async e=>{
  ...
  const legacyRecords=jsonFiles.find(f=>/^universe\s*records\.json$/i.test(f.name||''));
  ...
  try{
    if(!stock)errors.push('找不到 jmsdata.xlsx');else{const n=await importStockFile(stock);let migrated='';if(!state.dataMeta.records?.embedded&&legacyRecords){const r=await importLegacyUniverseRecordsFile(legacyRecords,{fallbackName:root});migrated=` · 已由舊 Universe Records.json 遷移 ${r.documents} 筆文件`;state.dataMeta.records.newTemplate=false}lines.push(`✓ ${stock.name} · ${n} 件貨品 · Available ${state.products.size} · Records ${officialDocumentCount()} 筆${state.dataMeta.records?.newTemplate?' · 新展覽空白記錄':''}${migrated} · 圖片選擇 ${state.imageOverrides.size} 項 · ${new Date(stock.lastModified).toLocaleString('zh-HK')}`)}

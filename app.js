...
function confirmStoredDocument(){
  ...
  if(!fxPricingReady())return alert(`請先取得或輸入 USD → ${currencyCode()} 的 FX Rate。`);
  if(!quote14KReady())return alert('14K 參考報價尚未取得 Kitco Ask，請先線上更新或手動輸入。');
  const type=state.documentType,documentNo=norm($('#invoiceNo').value)||formatDocumentNo();
  if(type==='quotation'){releaseCustomImages();state.items=[];state.quote.karat='18K';$$('input[name="quoteKarat"]').forEach(x=>x.checked=x.value==='18K');updateGoldQuoteUI();advanceDocumentSequence(documentNo,type);renderItems();status('#addMessage',`Quotation ${documentNo} 已 Confirm（庫存及 jmsdata 沒有改動）；下一張為 ${$('#invoiceNo').value}。`,'ok');return;}
  const keys=storeKeysForType(type),targetStatus=type==='consignment'?'CONSIGNED':'SOLD_ON_HAND',recall=state.recall&&state.recall.type===type&&state.recall.documentNo===documentNo?state.recall:null;
  if(!recall&&findLatestDocument(type,documentNo))return alert(`${documentNo} 已存在。請使用 Recall 重新開啟原文件，不要重複 Confirm 同一文件號碼。`);
  const revision=recall?recall.baseRevision+1:0,oldItems=recall?new Map(recall.items.map(x=>[String(docValue(x,'LOTNO')),x])):new Map(),newItems=formalItems(),newLots=new Set(newItems.map(x=>String(x.lotNo))),rollback=new Map();
  try{
    if(recall){for(const [lot,oldRow] of oldItems){if(newLots.has(lot))continue;const p=state.stockCatalog.get(lot)||{lotNo:lot,artNo:normArt(docValue(oldRow,'ARTNO'))},before=inventoryVectorFromRow(state.stockRowByLot.get(lot)||{}),original=vectorFromRecord(oldRow,'ORIGINAL'),currentStatus=inventoryStatusFromVector(before);if(currentStatus!==targetStatus)throw new Error(`${p.artNo} / LOTNO ${lot} 目前狀態是 ${historyStatusLabel(currentStatus)}，已不是這張文件原本的 ${historyStatusLabel(targetStatus)}；為避免覆蓋後續交易，不能直接從 Recall 刪除。`);if(!rollback.has(lot))rollback.set(lot,before);setInventoryVectorForLot(lot,original);appendTransaction({type,documentNo,revision,item:p,before,after:original})}}
    const itemRows=[];
    ...
    const previousCreated=recall?docValue(recall.header,'CREATED_AT'):'';state.documentStore[keys.headers].push(headerRowForCurrentDocument(type,documentNo,revision,previousCreated));state.documentStore[keys.items].push(...itemRows);saveDocumentStore();rebuildInventoryMaps();const exported=exportJmsdata();
    state.recall=null;setRecallLock(false);$('#recallActive')?.classList.add('hidden');clearCurrentDocument();advanceDocumentSequence(documentNo,type);renderRecallResults();const action=revision?'已儲存修訂':'已 Confirm';status('#addMessage',`${documentLabels(type).short} ${documentNo} ${action}（Revision ${revision}），${exported?'已輸出 jmsdata.xls':'jmsdata 匯出失敗'}；下一張為 ${$('#invoiceNo').value}。`,exported?'ok':'warn');status('#stockStatus',`目前 Available Stock：${state.products.size} 件。`,'ok');
...
updateFxPanel();updateDocumentTypeUI();updateGoldQuoteUI();renderCustomerSummary();renderItems();renderRecallResults();schedulePreview();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});

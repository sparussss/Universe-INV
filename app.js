...
const KITCO_CACHE_KEY='universeKitcoLondonPm_v2';
const KITCO_HISTORY_CACHE_KEY='universeKitcoLondonPmHistory_v1';
const KITCO_LONDON_HISTORY_PAGE='https://www.kitco.com/charts/historicalgold.html';
...
function quote14KMode(){return quoteMode()&&state.quote.karat==='14K'}
...
function goldMetrics(londonPm=state.quote.kitcoAsk){const ask=Number(londonPm)||0;if(!(ask>0))return null;const base=companyGoldPrice(ask),g18=goldValuePerGram(base,.75),g14=goldValuePerGram(base,.585);return{ask,base,g18,g14}}
function historicalLondonPm(item){const date=normalizeStockDate(item?.completionDate),n=Number(state.quote.historicalPm?.[date]);return date&&Number.isFinite(n)&&n>0?n:0}
function quotationPriceDetails(item){
  const p=parse18KDesc1((item?.descriptions||[])[0]),date=normalizeStockDate(item?.completionDate),current=goldMetrics(),historicalPm=historicalLondonPm(item),historical=goldMetrics(historicalPm),rate=Number($('#salesRate')?.value)||0;
  const missing=[];if(!p)missing.push('DESC1 金重');if(!date)missing.push('LDATE');if(!current)missing.push('最新 London PM');if(date&&!historical)missing.push(`${date} London PM`);
  if(missing.length)return{ready:false,missing,date,p,current,historical,historicalPm};
  const currentWeight=quote14KMode()?roundUpWeight05(p.weight*K14_WEIGHT_FACTOR):p.weight,currentPerGram=quote14KMode()?current.g14:current.g18;
  ...
  return{ready:true,date,p,current,historical,historicalPm,currentWeight,currentPerGram,historicalTotal,currentTotal,adjustment,rawBase,rawPrice,finalPrice};
}
function quotationGoldReady(){return !quoteMode()||!!goldMetrics()&&state.items.every(item=>quotationPriceDetails(item).ready)}
...
function activePriceOverrides(item){item.currencyPrices=item.currencyPrices||{};item.quote18kCurrencyPrices=item.quote18kCurrencyPrices||{};item.quote14kCurrencyPrices=item.quote14kCurrencyPrices||{};return quoteMode()?(quote14KMode()?item.quote14kCurrencyPrices:item.quote18kCurrencyPrices):item.currencyPrices}
function saveKitcoCache(){try{if(state.quote.kitcoAsk>0)localStorage.setItem(KITCO_CACHE_KEY,JSON.stringify({ask:state.quote.kitcoAsk,time:state.quote.kitcoTime||'',fetchedAt:Date.now()}));else localStorage.removeItem(KITCO_CACHE_KEY)}catch{}}
...
function renderHistoricalGoldInputs(){
  ...
  if(!state.items.length){box.innerHTML='<div class="notice">加入 Quotation 貨品後，這裡會按 Column R（LDATE）列出需要輸入的完成日 London PM。</div>';return}
  box.innerHTML='';
  for(const [date,count] of dates){
    const value=Number(state.quote.historicalPm?.[date])||0,company=companyGoldPrice(value),row=document.createElement('div');row.className='historical-gold-row';row.innerHTML=`<div class="historical-gold-date"><strong>${esc(date)}</strong><span>${count} 款 · 完成日</span></div><label>London PM<input type="number" min="0" step="0.01" inputmode="decimal" data-gold-date="${esc(date)}" value="${value>0?value:''}" placeholder="例如 4062.20"></label><div class="historical-company-price"><span>公司金價</span><strong>${company?`USD ${company.toFixed(0)} / oz`:'—'}</strong></div>`;
...
function updateGoldQuoteUI(message='',type=''){
  ...
  const m=goldMetrics();$('#companyGoldBase').textContent=m?`USD ${m.base.toFixed(0)} / oz`:'—';$('#gold18PerGram').textContent=m?`USD ${m.g18.toFixed(3)}`:'—';$('#gold14PerGram').textContent=m?`USD ${m.g14.toFixed(3)}`:'—';renderHistoricalGoldInputs();
...
function openKitcoLondonHistory(){window.open(KITCO_LONDON_HISTORY_PAGE,'_blank','noopener,noreferrer');updateGoldQuoteUI('已開啟 Kitco 金價頁；請把最新及完成日 London PM 輸入上方欄位。','warn')}
function setQuoteKarat(value){state.quote.karat=value==='14K'?'14K':'18K';$$('input[name="quoteKarat"]').forEach(x=>x.checked=x.value===state.quote.karat);syncEffectivePrices({clearCurrentOverride:true});updateGoldQuoteUI();renderItems();schedulePreview()}
loadInventoryHistory();loadDocumentStore();loadKitcoCache();loadHistoricalGoldCache();
...
function addProductToDocument(p,{fromSearch=false}={}){
  ...
  const rate=Number($('#salesRate').value)||0,usdUnitPrice=Math.ceil((Number(p.price)||0)*rate),match=chooseImageMatch(p);const item={...productSnapshot(p),id:Date.now()+Math.random(),seq:state.items.length+1,qty:1,usdUnitPrice,currencyPrices:{},quote18kCurrencyPrices:{},quote14kCurrencyPrices:{},unitPrice:0,imageVariant:match.variant,imageGrayscale:!!match.grayscale,imageAutoMatched:!match.manualOverride,imageOverrideFile:match.fileName||''};state.items.push(item);item.unitPrice=convertedFromUsd(effectiveUsdPrice(item));
...
function renderItems(){
  ...
  if(!state.items.length){state.editingItemId=null;box.className='invoice-items empty-state';box.textContent='尚未加入貨品。';renderHistoricalGoldInputs();updateTotals();updateBackToTopButton();return}
  ...
  displayItems().forEach(item=>{
    ...
    if(quoteMode()){
      const q=quotationPriceDetails(item),karat=quote14KMode()?'14K':'18K';
      if(q.ready){
        const adjustment=`${q.adjustment>=0?'+':'−'}${fmt(Math.abs(q.adjustment),'USD')}`;
        if(quote14KMode())note.textContent=code==='USD'?`14K：${q.p.weight.toFixed(2)}g → ${q.currentWeight.toFixed(2)}g · 完成日18K ${fmt(q.historicalTotal,'USD')} → 最新14K ${fmt(q.currentTotal,'USD')} · 調整 ${adjustment} → ${fmt(q.finalPrice,'USD')}`:(fxPricingReady()?`14K：USD ${q.finalPrice.toFixed(0)} × ${currentFxRate()} → ${fmt(item.unitPrice,code)}`:`14K：USD ${q.finalPrice.toFixed(0)} · 等待 FX Rate`);
        else note.textContent=code==='USD'?`18K：完成日公司金價 USD ${q.historical.base.toFixed(0)} → 最新 USD ${q.current.base.toFixed(0)} · 調整 ${adjustment} → ${fmt(q.finalPrice,'USD')}`:(fxPricingReady()?`18K：USD ${q.finalPrice.toFixed(0)} × ${currentFxRate()} → ${fmt(item.unitPrice,code)}`:`18K：USD ${q.finalPrice.toFixed(0)} · 等待 FX Rate`);
      }else note.textContent=`${karat} Quotation：等待 ${q.missing.join('／')}`;
      note.classList.add('quote14-price-note');if(quote14KMode()){const badge=document.createElement('span');badge.className='quote14-badge';badge.textContent='14K';$('.item-artno',node).after(badge)}
    ...
    $('.price-input',node).onchange=e=>{const code=currencyCode(),raw=Math.max(0,Number(e.target.value)||0),value=code==='EUR'?Math.round(raw):(code==='USD'&&quoteMode()?Math.ceil(raw-1e-9):roundUnitPrice(raw,code)),overrides=activePriceOverrides(item);overrides[code]=value;item.unitPrice=value;if(code==='USD'&&!quoteMode())item.usdUnitPrice=value;e.target.value=value;updateTotals()};
  ...
  renderHistoricalGoldInputs();updateTotals();updateBackToTopButton()
}
...
async function exportInvoiceExcel(){
  ...
  if(!quote14KReady())return alert('Quotation 金價資料未完成。請輸入最新 Kitco London PM，並補齊所有貨品完成日的 London PM。');
...
function confirmStoredDocument(){
  ...
  if(state.items.length&&!quote14KReady())return alert('Quotation 金價資料未完成。請輸入最新 Kitco London PM，並補齊所有貨品完成日的 London PM。');
...
function renderStockSearch(){
  ...
  for(const x of shown){
    ...
    card.innerHTML=`<div class="stock-result-image-wrap"><div class="stock-result-thumb-box"><img class="${grayClass}" src="${esc(display.src)}" alt="${esc(x.artNo)}">${imageBadge}</div></div><div><h4>${esc(x.artNo)}</h4><div class="lot">LOTNO ${esc(x.lotNo)}</div><div class="desc">${esc((x.descriptions||[]).join('\n'))}</div></div><div class="stock-result-actions"><span class="stock-status ${historyStatusClass(x.status)}">${esc(historyStatusLabel(x.status))}</span>${inDoc?'<span class="stock-current-doc">已在目前文件</span>':''}</div>`;
...
$$('input[name="quoteKarat"]').forEach(r=>r.onchange=e=>setQuoteKarat(e.target.value));$('#refreshGoldBtn').onclick=openKitcoLondonHistory;let goldInputTimer=null;$('#kitcoAskInput').oninput=e=>{clearTimeout(goldInputTimer);goldInputTimer=setTimeout(()=>{const n=Number(e.target.value);if(n>0)setKitcoAsk(n,{source:'manual',message:`最新 Kitco London PM：USD ${n.toFixed(2)} / oz`});else{state.quote.kitcoAsk=0;saveKitcoCache();syncEffectivePrices({clearCurrentOverride:true});renderItems();updateGoldQuoteUI('請輸入大於 0 的最新 London PM。','error')}},180)};

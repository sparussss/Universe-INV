...
function saveExhibitionName(value=state.exhibitionName){
  state.exhibitionName=norm(value);try{if(state.exhibitionName)localStorage.setItem(EXHIBITION_NAME_KEY,state.exhibitionName);else localStorage.removeItem(EXHIBITION_NAME_KEY)}catch{}
  const input=$('#exhibitionName');if(input&&document.activeElement!==input)input.value=state.exhibitionName;
  const current=$('#currentExhibitionStatus');if(current)current.textContent=state.exhibitionName?`目前展覽：${state.exhibitionName}`:'目前展覽：尚未命名';const workspace=$('#workspaceExhibitionName');if(workspace)workspace.textContent=state.exhibitionName?` · ${state.exhibitionName}`:'';updateDataVersionPanel();
}
function syncExhibitionNameFromInput(){saveExhibitionName($('#exhibitionName')?.value||'')}
function resetExhibitionSequences(){
  if(state.recall)return alert('Recall 修改期間不能重設文件編號；請先完成或取消 Recall。');
  const yy=invoiceYear(),hasDraft=state.items.length||norm($('#customerCode')?.value)||norm($('#customerName')?.value);
  const suggested=state.exhibitionName||state.packageName||'';
  const entered=prompt('請輸入新的內部 Exhibition Name。\n只供 PWA 管理及 Recall 分組，不會印在文件上。',suggested);
  if(entered===null)return;
  const exhibitionName=norm(entered);if(!exhibitionName)return alert('請輸入 Exhibition Name。');
  const extra=hasDraft?'\n\n目前畫面有未完成資料；重設只會改文件號碼，不會清除貨品或客戶資料。':'';
  if(!confirm(`開始新展覽會「${exhibitionName}」？\nInvoice / Consignment / Quotation 的下一個文件編號會重設為 ${yy}0001。\n\nINV${yy}0001 / CON${yy}0001 / QUO${yy}0001${extra}`))return;
  state.exhibitionSession=`EXH${Date.now()}`;try{localStorage.setItem(EXHIBITION_SESSION_KEY,state.exhibitionSession)}catch{}saveExhibitionName(exhibitionName);
  for(const type of ['invoice','consignment','quotation'])localStorage.setItem(documentSequenceKey(type,yy),'1');
  $('#invoiceNo').value=formatDocumentNo(1,yy,state.documentType);schedulePreview();if(hasDraft)scheduleDraftSave();else{clearDocumentDraft();markDraftBaseline()};renderRecallResults();
  status('#addMessage',`新展覽會「${exhibitionName}」文件編號已重設：INV${yy}0001 / CON${yy}0001 / QUO${yy}0001。舊文件記錄不會改動。`,'ok');
}
...
function importImageOverridesFromWorkbook(wb){
  ...
  let merge=true;if(conflicts.length)merge=confirm(`更新資料包時發現 ${conflicts.length} 項 Image Override 與本機記錄不同。\n\n按「確定」：保留本機選圖並補入資料包內其他記錄。\n按「取消」：保留全部本機 Image Override，不匯入衝突資料。`);
...
function updateImageOverrideStatus(){
  ...
  const exportJms=$('#exportImageOverridesBtn'),exportPackage=$('#exportUpdatedPackageBtn');
  if(exportJms)exportJms.disabled=!state.stockAllRows.length;
  if(exportPackage)exportPackage.disabled=!state.stockAllRows.length||!state.packageFiles.length||typeof JSZip==='undefined';
  if(!el)return;
  if(dirty||newImages){
    const parts=[];if(dirty)parts.push(`${dirty} 項選圖`);if(newImages)parts.push(`${newImages} 張新增圖片`);
    el.textContent=`尚有 ${parts.join('、')} 未匯出。完成整理後請匯出更新後資料包。`;el.className='notice warn';
...
function confirmDiscardPendingImageOverrides(){const pending=state.imageOverrideDirty+(state.packageImageDirtyFiles?.size||0);return !pending||confirm(`尚有 ${pending} 項圖片修改未匯出。\n\n更新資料包會保留本機 Image Override 記錄，但新上傳／拍攝而尚未寫入資料包的圖片檔本身可能無法帶到另一個 Folder。建議先匯出更新後資料包或 PWA 備份。\n\n仍然繼續？`)}
function saveImageOverride(product,image,grayscale=false){
  ...
  applyImageOverrideToCurrentItem(product);saveImageOverridesLocal();updateImageOverrideStatus();renderStockSearch();renderItems();runHealthCheck();schedulePreview();
}
function clearImageOverride(product){
  ...
  state.imageOverrideDirtyLots.add(key);state.imageOverrideDirty=state.imageOverrideDirtyLots.size;applyImageOverrideToCurrentItem(product);saveImageOverridesLocal();updateImageOverrideStatus();renderStockSearch();renderItems();runHealthCheck();schedulePreview();
}
...
function syncDocumentSequencesFromStore(){const yy=invoiceYear();for(const type of ['invoice','consignment','quotation']){const prefix=documentPrefix(type);let next=getNextDocumentSequence(type,yy);for(const h of latestDocumentHeaders(type,{session:state.exhibitionSession})){const no=norm(docValue(h,'DOCUMENT_NO')).toUpperCase(),m=no.match(new RegExp(`^${prefix}(\\d{2})(\\d{4})$`));if(m&&m[1]===yy)next=Math.max(next,Number(m[2])+1)}localStorage.setItem(documentSequenceKey(type,yy),String(next))}if(!state.recall&&!state.items.length)setDefaultInvoiceNo(true)}
function importDocumentStoreFromWorkbook(wb){
  ...
  let useMerged=true;if(conflicts.length)useMerged=confirm(`更新資料包時發現 ${conflicts.length} 項本機文件記錄與 jmsdata 同一 Revision 內容不同。\n\n按「確定」：合併資料，Header 以較新 UPDATED_AT 為準，其他衝突保留本機版本。\n按「取消」：只更新庫存資料，不匯入 jmsdata 內的文件記錄。`);
...
function packageFilePath(file){return String(file?.webkitRelativePath||file?.name||'').replace(/^\/+/, '')}
function packagePicturesFolder(){const sample=(state.packageFiles||[]).find(f=>/\/(?:pictures?|images?)\//i.test('/'+packageFilePath(f))&&(String(f.type||'').startsWith('image/')||/\.(jpe?g|png|webp)$/i.test(f.name||'')));if(sample){const path=packageFilePath(sample);return path.slice(0,path.lastIndexOf('/'))}return `${state.packageName||'Exhibition Package'}/Pictures`}
async function exportUpdatedPackage(){
  if(!state.packageFiles.length)return alert('請先使用「匯入展覽資料包」載入完整 Folder。');
  if(typeof JSZip==='undefined')return alert('資料包輸出程式未載入，請連接網絡後重新開啟 PWA。');
  const btn=$('#exportUpdatedPackageBtn');if(btn){btn.disabled=true;btn.textContent='正在建立資料包…'}
  try{
    const zip=new JSZip(),jmsBuffer=XLSX.write(buildJmsdataWorkbook(),{bookType:'xls',type:'array'}),jmsOriginal=(state.packageFiles||[]).find(f=>/^jmsdata(?:\s.*)?\.(xls|xlsx)$/i.test(f.name)||/jmsdata/i.test(f.name)),jmsPath=jmsOriginal?packageFilePath(jmsOriginal):`${state.packageName||'Exhibition Package'}/jmsdata.xls`;
    const referencedNames=new Set([...state.imageOverrides.values()].map(row=>String(row.IMAGE_FILE||'').toUpperCase()).filter(Boolean)),replacementNames=new Set([...state.customPackageImages.keys()].filter(name=>referencedNames.has(name)));
    for(const file of state.packageFiles){const path=packageFilePath(file);if(!path)continue;if(file===jmsOriginal)continue;if(replacementNames.has(String(file.name||'').toUpperCase()))continue;zip.file(path,file)}
    zip.file(jmsPath,jmsBuffer);
    const pictureDir=packagePicturesFolder();for(const [name,file] of state.customPackageImages){if(referencedNames.has(name))zip.file(`${pictureDir}/${file.name}`,file)};
    const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:3}},meta=>{if(btn)btn.textContent=`建立資料包 ${Math.round(meta.percent)}%`});
    downloadBlob(blob,`${state.packageName||'Exhibition Package'}_Updated.zip`);state.imageOverrideDirtyLots=new Set();state.imageOverrideDirty=0;state.packageImageDirtyFiles=new Set();updateImageOverrideStatus();
  }catch(err){console.error(err);alert('匯出更新後資料包失敗：'+(err.message||err))}finally{if(btn){btn.textContent='匯出更新後資料包';updateImageOverrideStatus()}}
}
...
function renderRecallResults(){
  ...
  for(const type of types){for(const h of latestDocumentHeaders(type)){const no=norm(docValue(h,'DOCUMENT_NO')),customer=norm(docValue(h,'CUSTOMER')),code=norm(docValue(h,'CUSTOMER_CODE')),date=norm(docValue(h,'DOCUMENT_DATE')),exhibition=norm(docValue(h,'EXHIBITION_NAME')),hay=`${no} ${customer} ${code} ${date} ${exhibition}`.toUpperCase();if(q&&!hay.includes(q))continue;const session=recordSession(h),found=findLatestDocument(type,no,session);rows.push({type,h,session,count:found?.items.length||0,exhibition:exhibition||session||'未命名展覽'})}}
...
function updateDraftStatus(text='',kind=''){const el=$('#draftStatus');if(!el)return;if(!text){el.textContent='';el.className='draft-status';return}el.textContent=text;el.className='draft-status'+(kind?' '+kind:'')}
...
function syncDraftAfterDataImport(){if(loadDocumentDraft())return maybeOfferDraftRestore();markDraftBaseline();updateDraftStatus();return false}
...
async function importStockFile(f){
  ...
  state.stockWorkbook=wb;state.stockSheetName=sheetName||'jmsdata';state.stockFileName=f.name||'jmsdata.xls';state.stockHeaders=Object.keys(rows[0]||{});state.stockAllRows=rows;state.stockRowByLot=new Map();state.stockIntegrityIssues=[];state.stockDuplicateLots=[];
  ...
  if(!fullMap.size)throw new Error('找不到有效 LOTNO / ARTNO / PRICE');state.stockCatalog=fullMap;const mergeInfo=importDocumentStoreFromWorkbook(wb);const imageInfo=importImageOverridesFromWorkbook(wb);rebuildInventoryMaps();state.dataMeta.stock={name:f.name||'',lastModified:Number(f.lastModified)||0,count:fullMap.size,available:state.products.size,duplicates:state.stockDuplicateLots.length,integrity:state.stockIntegrityIssues.length,documentConflicts:mergeInfo?.conflicts||0,imageOverrideConflicts:imageInfo?.conflicts||0};for(const id of ['#exportJmsdataBtn','#exportImageOverridesBtn','#exportUpdatedPackageBtn']){const exportBtn=$(id);if(exportBtn)exportBtn.disabled=false}renderRecallResults();updateDataVersionPanel();runHealthCheck();return fullMap.size;
}
...
function healthSummaryRows(){
  ...
  if(state.importConflicts?.length)rows.push({level:'warn',text:`更新資料包：偵測到 ${state.importConflicts.length} 項文件記錄衝突，已按匯入時選擇處理。`});
...
function exportPwaBackup(){saveDocumentDraft();saveDocumentStore();saveInventoryHistory();saveImageOverridesLocal();syncExhibitionNameFromInput();const payload={app:'Universe Invoice',backupVersion:1,appVersion:APP_VERSION,exportedAt:new Date().toISOString(),exhibitionSession:state.exhibitionSession||'',exhibitionName:state.exhibitionName||'',localStorage:backupLocalStorageSnapshot(),dataMeta:state.dataMeta||{},notes:'External Excel/Pictures files are not embedded. Re-import the exhibition package after restore.'};downloadBlob(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),`UniverseInvoice_PWA_Backup_${backupFileStamp()}.json`);status('#backupStatus','PWA 備份已匯出。備份包含本機歷史、Recall、草稿、Image Overrides、文件流水號及 Exhibition Session／Name；原始 Excel／Pictures 需另外保存。','ok')}
...
$('#exhibitionPackageInput').onchange=async e=>{
  ...
  try{
    if(!stock)errors.push('找不到 jmsdata.xls / jmsdata.xlsx');else{const n=await importStockFile(stock);lines.push(`✓ ${stock.name} · ${n} 件貨品 · Available ${state.products.size} · 文件記錄 ${state.documentStore.invoiceHeaders.length+state.documentStore.consignmentHeaders.length+state.documentStore.quotationHeaders.length} 筆 · 圖片選擇 ${state.imageOverrides.size} 項 · ${new Date(stock.lastModified).toLocaleString('zh-HK')}`)}
    ...
    if(!imageFiles.length){errors.push('找不到 Pictures 內的圖片');delete state.dataMeta.images}else{const r=await importImageFiles(imageFiles);for(const item of state.items)if(!item.customImage)applyAutoImageMatch(item);lines.push(`✓ Pictures · ${r.images} 張圖片 · 配對 ${r.matched} 個款號`)}
...
$('#clearInvoiceBtn').onclick=()=>{if(confirm(`清空目前 ${documentLabels().short} 貨品？`)){releaseCustomImages();state.items=[];renderItems();if(state.recall)scheduleDraftSave();else{clearDocumentDraft();markDraftBaseline()}}};
...
function confirmStoredDocument(){
  ...
  try{
    if(recall&&!isQuote){for(const [lot,oldRow] of oldItems){if(newLots.has(lot))continue;const p=state.stockCatalog.get(lot)||{lotNo:lot,artNo:normArt(docValue(oldRow,'ARTNO'))},before=inventoryVectorFromRow(state.stockRowByLot.get(lot)||{}),original=vectorFromRecord(oldRow,'ORIGINAL'),currentStatus=inventoryStatusFromVector(before);if(currentStatus!==targetStatus)throw new Error(`${p.artNo} / LOTNO ${lot} 目前狀態是 ${historyStatusLabel(currentStatus)}，已不是這張文件原本的 ${historyStatusLabel(targetStatus)}；如貨品已 Delivered，請先在配套搜尋使用「取消已交貨」。`);if(!rollback.has(lot))rollback.set(lot,before);setInventoryVectorForLot(lot,original);appendTransaction({type,documentNo,revision,item:p,before,after:original})}}
    const itemRows=[];
    for(const item of newItems){const lot=String(item.lotNo),row=state.stockRowByLot.get(lot);if(!row)throw new Error(`jmsdata 找不到 LOTNO ${lot}`);const before=inventoryVectorFromRow(row),oldRow=oldItems.get(lot),isExisting=!!oldRow;let original=isExisting?vectorFromRecord(oldRow,'ORIGINAL'):(item._originalVector||before),after=before;
      if(!isQuote){const currentStatus=inventoryStatusFromVector(before);if(isExisting){if(currentStatus===targetStatus)after=inventoryVectorForStatus(targetStatus,before.m);else if(type==='invoice'&&currentStatus==='SOLD_DELIVERED')after=before;else throw new Error(`${item.artNo} / LOTNO ${lot} 的庫存狀態已在這張文件之後改為 ${historyStatusLabel(currentStatus)}，不能由舊文件 Recall 覆蓋。`)}else after=inventoryVectorForStatus(targetStatus,before.m);if(!isExisting&&inventoryStatusFromVector(before)!=='AVAILABLE')throw new Error(`${item.artNo} / LOTNO ${lot} 目前不是 Available，不能新增到 ${documentLabels(type).short}。`);if(!rollback.has(lot))rollback.set(lot,before);setInventoryVectorForLot(lot,after);appendTransaction({type,documentNo,revision,item,before,after})}
      itemRows.push(itemRowForDocument(item,documentNo,revision,original,after));
    }
    const previousCreated=recall?docValue(recall.header,'CREATED_AT'):'';state.documentStore[keys.headers].push(headerRowForCurrentDocument(type,documentNo,revision,previousCreated,documentStatus));state.documentStore[keys.items].push(...itemRows);saveDocumentStore();if(!isQuote)rebuildInventoryMaps();const exported=exportJmsdata(),wasRecall=!!recall;
    state.recall=null;setRecallLock(false);$('#recallActive')?.classList.add('hidden');clearDocumentDraft();clearCurrentDocument();if(isQuote){state.quote.karat='18K';$$('input[name="quoteKarat"]').forEach(x=>x.checked=x.value==='18K');updateGoldQuoteUI()}if(wasRecall)setDefaultInvoiceNo(true);else advanceDocumentSequence(documentNo,type);markDraftBaseline();renderRecallResults();const action=documentStatus==='CANCELLED'?'已取消整張文件':revision?'已儲存修訂':'已 Confirm',inventoryNote=isQuote?'庫存不變；':' ';status('#addMessage',`${documentLabels(type).short} ${documentNo} ${action}（Revision ${revision}），${inventoryNote}${exported?'已輸出 jmsdata.xls':'jmsdata 匯出失敗'}；目前文件號碼為 ${$('#invoiceNo').value}。`,exported?'ok':'warn');if(!isQuote)status('#stockStatus',`目前 Available Stock：${state.products.size} 件。`,'ok');
...
function confirmPreflightError(){
  ...
  if(!state.stockAllRows.length)return'請先匯入 jmsdata。';
...
function openFinalConfirmDialog(){
  ...
  const rows=[['Document No.',norm($('#invoiceNo').value)],['Customer',`${norm($('#customerCode').value)} · ${norm($('#customerName').value)}`],['Sales Rate',norm($('#salesRate').value)],['Currency',currencyCode()],['Quantity',String(t.qty)],['Sub Total',fmt(t.sub)],['Discount',t.discount?`(${fmt(t.discount)})`:fmt(0)],['Total',fmt(t.total)],['圖片待處理',String(img.issues)],['人手改價',String(manual)]];
...
function validStoneBreakdownCodes(){return [...activeStoneAliases().keys()].map(x=>norm(x).toUpperCase().replace(/\s+/g,'')).filter(Boolean).sort((a,b)=>b.length-a.length||a.localeCompare(b))}
...
function stockStoneSignatureCodes(p){return stockSortStoneCodes(p,[])}
...
function renderStockSearch(){
  ...
  if(!records.length){resultsEl.innerHTML='';$('#stockSearchMessage').textContent='尚未匯入 jmsdata。';$('#stockTypeFilters').classList.add('hidden');$('#stockStoneFilters').classList.add('hidden');$('#stockSearchSummary').classList.add('hidden');filterControls?.classList.add('hidden');filterPanel?.classList.add('hidden');return}

...
const state={
  ...
  insertAt:null, scanner:null, scannerRunning:false, scannerPaused:false,
  lastScan:{value:'',time:0}, installPrompt:null
};
...
$('#excelInput').addEventListener('change',async e=>{
  ...
  try{
    ...
    for(const row of rows){
      ...
      const descriptions=[]; for(let i=1;i<=6;i++){const v=normalizeKey(field(row,[`DESC${i}`,`DESCRIPTION${i}`]));if(v)descriptions.push(v)}
      map.set(lot,{lotNo:lot,artNo:art,price,unit:normalizeKey(field(row,['UNIT']))||'PC',descriptions});
    }
...
function rebuildImageIndex(){
  ...
  renderImageLibrary();renderItems();
}
...
function addByLot(raw){
  ...
  const item={id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),...product,qty:1,unitPrice,imageVariant:imgs.find(x=>x.variant==='Default')?.variant||imgs[0]?.variant||'Default'};
...
function renderItems(){
  ...
  state.items.forEach((item,index)=>{
    ...
    sel.addEventListener('change',()=>{item.imageVariant=sel.value;renderItems();persistLight()});
    ...
    $('.insert-above',node).addEventListener('click',()=>{state.insertAt=index;$('#lotInput').focus();setStatus('#addMessage',`下一件會插入第 ${index+1} 款之前。`,'ok')});
    $('.insert-below',node).addEventListener('click',()=>{state.insertAt=index+1;$('#lotInput').focus();setStatus('#addMessage',`下一件會插入第 ${index+1} 款之後。`,'ok')});
    $('.delete-item',node).addEventListener('click',()=>{if(confirm(`刪除 ${item.artNo} / LOTNO ${item.lotNo}？`)){state.items.splice(index,1);renderItems();persistLight()}});
    node.addEventListener('dragstart',e=>e.dataTransfer.setData('text/plain',String(index)));node.addEventListener('dragover',e=>e.preventDefault());node.addEventListener('drop',e=>{e.preventDefault();const from=Number(e.dataTransfer.getData('text/plain'));const [m]=state.items.splice(from,1);state.items.splice(index,0,m);renderItems();persistLight()});
    box.appendChild(node);
  });updateTotals();
}
$('#clearInvoiceBtn').addEventListener('click',()=>{if(state.items.length&&confirm('清空目前 Invoice 草稿？')){state.items=[];renderItems();persistLight()}});
...
$('#confirmInvoiceBtn').addEventListener('click',()=>{
  ...
  try{
    ...
    state.items=[];$('#discountAmount').value='0';renderItems();rebuildImageIndex();persistLight();renderPreview();
...
async function startScanner(){
  const dlg=$('#scannerDialog');dlg.showModal();$('#scannerStatus').textContent='正在啟動後置相機…';
  ['#pauseScannerBtn','#torchBtn','#zoom1Btn','#zoom2Btn','#zoom3Btn','#zoom4Btn'].forEach(x=>$(x).disabled=true);
  if(typeof Html5Qrcode==='undefined')return $('#scannerStatus').textContent='掃描程式未載入。請連接網絡並重新開啟。';
  try{
    state.scanner=state.scanner||new Html5Qrcode('reader',{verbose:false});
    ...
    await state.scanner.start({facingMode:'environment',width:{ideal:3840},height:{ideal:2160}},{fps:20,qrbox:(w,h)=>({width:Math.floor(w*.9),height:Math.max(65,Math.min(90,Math.floor(h*.13)))}),aspectRatio:1.7778,formatsToSupport:formats,experimentalFeatures:{useBarCodeDetectorIfSupported:true}},onBarcodeSuccess,()=>{});
    state.scannerRunning=true;state.scannerPaused=false;$('#pauseScannerBtn').disabled=false;$('#scannerStatus').textContent='請把條碼橫向填滿中央細長框。';
    const caps=state.scanner.getRunningTrackCapabilities?.();if(caps?.torch)$('#torchBtn').disabled=false;
    if(caps?.zoom){for(const z of [1,2,3,4])$(`#zoom${z}Btn`).disabled=z>(caps.zoom.max||1);await setZoom(Math.min(4,caps.zoom.max||1))}
    try{await state.scanner.applyVideoConstraints({advanced:[{focusMode:'continuous'}]})}catch{}
  }catch(err){state.scannerRunning=false;$('#scannerStatus').textContent=`相機無法啟動：${err?.message||err}`}
}
...
async function setZoom(value){if(!state.scannerRunning)return;try{await state.scanner.applyVideoConstraints({advanced:[{zoom:value}]});for(const z of [1,2,3,4])$(`#zoom${z}Btn`).classList.toggle('active',z===value);$('#scannerStatus').textContent=`已切換 ${value}×。請等相機清晰對焦。`}catch{$('#scannerStatus').textContent=`此裝置不支援 ${value}× 網頁相機變焦。`}}
...
$('#photoScanBtn').addEventListener('click',()=>$('#photoScanInput').click());$('#photoScanInput').addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;if(typeof Html5Qrcode==='undefined')return setStatus('#addMessage','掃描程式未載入。','error');let scanner;try{scanner=new Html5Qrcode('reader',{verbose:false});const result=await scanner.scanFile(file,true);$('#lotInput').value=cleanScannedLot(result);addByLot(result)}catch{setStatus('#addMessage','照片內未能解讀 Barcode。可先在相片 App 裁切／放大條碼後再選取。','error')}finally{e.target.value='';try{scanner?.clear()}catch{}}});

function persistLight(){try{localStorage.setItem('ui-v04',JSON.stringify({customerCode:$('#customerCode').value,customerName:$('#customerName').value,customerAddress:$('#customerAddress').value,salesRate:$('#salesRate').value,currency:$('#currency').value,shipmentMethod:$('#shipmentMethod').value,customerTerms:$('#customerTerms').value,invoiceNo:$('#invoiceNo').value,invoiceDate:$('#invoiceDate').value,discount:$('#discountAmount').value,remark:$('#remark').value,items:state.items,soldLots:[...state.soldLots.entries()]}))}catch{}}
function restoreLight(){try{const d=JSON.parse(localStorage.getItem('ui-v04')||'null');if(!d)return;for(const[id,key]of[['#customerCode','customerCode'],['#customerName','customerName'],['#customerAddress','customerAddress'],['#salesRate','salesRate'],['#currency','currency'],['#shipmentMethod','shipmentMethod'],['#customerTerms','customerTerms'],['#invoiceNo','invoiceNo'],['#invoiceDate','invoiceDate'],['#discountAmount','discount'],['#remark','remark']])if(d[key]!==undefined)$(id).value=d[key];if(Array.isArray(d.items))state.items=d.items;if(Array.isArray(d.soldLots))state.soldLots=new Map(d.soldLots)}catch{}}

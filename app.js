...
function stoneCodesForProduct(p){const text=(p.descriptions||[]).join(' ').toUpperCase(),known=new Set([...activeStoneAliases().keys(),'PSA','MCT','RGT','GGTR','GGT','SAR','BSA']);const out=[];for(const c of [...known].map(x=>norm(x).toUpperCase()).filter(Boolean).sort((a,b)=>b.length-a.length)){if(c==='CDM')continue;if(text.includes(c)&&!out.includes(c))out.push(c)}return out}
...
function primaryStoneCodeForProduct(p){const text=(p.descriptions||[]).join(' ').toUpperCase(),codes=stoneCodesForProduct(p);if(!codes.length)return'';const scored=codes.map(code=>({code,pos:text.indexOf(code)}));scored.sort((a,b)=>(a.pos<0?999999:a.pos)-(b.pos<0?999999:b.pos)||b.code.length-a.code.length||stoneOrderRank(a.code)-stoneOrderRank(b.code));return scored[0]?.code||''}
...
function stockFilterSummaryText(){const t=selectedStockFilters('type'),s=selectedStockFilters('stone');return `款式：${t.length?t.join(', '):'全部'} · 石頭：${s.length?s.join(', '):'全部'}`}
function runStockSearch(){const raw=norm($('#stockSearchInput').value).toUpperCase(),core=raw==='*'?'*':articleCore(raw);state.stockSearch.query=core||raw;state.stockSearch.types=[];state.stockSearch.stones=[];state.stockSearch.filtersOpen=false;renderStockSearch()}

...
function resetExhibitionSequences(){
  ...
  state.exhibitionSession=`EXH${Date.now()}`;try{localStorage.setItem(EXHIBITION_SESSION_KEY,state.exhibitionSession)}catch{}
  for(const type of ['invoice','consignment','quotation'])localStorage.setItem(documentSequenceKey(type,yy),'1');
  $('#invoiceNo').value=formatDocumentNo(1,yy,state.documentType);schedulePreview();if(hasDraft)scheduleDraftSave();else{clearDocumentDraft();markDraftBaseline()};
  status('#addMessage',`新展覽會文件編號已重設：INV${yy}0001 / CON${yy}0001 / QUO${yy}0001。舊文件記錄不會改動。`,'ok');
}
setDefaultInvoiceNo();
...
function clearImageOverride(product){
  ...
  state.imageOverrideDirtyLots.add(key);state.imageOverrideDirty=state.imageOverrideDirtyLots.size;applyImageOverrideToCurrentItem(product);updateImageOverrideStatus();renderStockSearch();renderItems();schedulePreview();
}
...
function importDocumentStoreFromWorkbook(wb){const store=emptyDocumentStore();for(const [key,name] of Object.entries(DOCUMENT_SHEETS))store[key]=sheetRows(wb,name);state.documentStore=normalizeDocumentStore(store);if(!state.exhibitionSession){const headers=[...state.documentStore.invoiceHeaders,...state.documentStore.consignmentHeaders,...state.documentStore.quotationHeaders].filter(x=>recordSession(x)).sort((a,b)=>String(docValue(b,'UPDATED_AT')||docValue(b,'DOCUMENT_DATE')).localeCompare(String(docValue(a,'UPDATED_AT')||docValue(a,'DOCUMENT_DATE')))),session=headers.length?recordSession(headers[0]):'';if(session){state.exhibitionSession=session;try{localStorage.setItem(EXHIBITION_SESSION_KEY,session)}catch{}}}saveDocumentStore();syncDocumentSequencesFromStore();renderRecallResults()}
...
function renderRecallResults(){const box=$('#recallResults');if(!box)return;const q=norm($('#recallSearchInput')?.value).toUpperCase(),typeFilter=$('#recallTypeFilter')?.value||'all',types=typeFilter==='all'?['invoice','consignment','quotation']:[typeFilter],rows=[];for(const type of types){for(const h of latestDocumentHeaders(type)){const no=norm(docValue(h,'DOCUMENT_NO')),customer=norm(docValue(h,'CUSTOMER')),code=norm(docValue(h,'CUSTOMER_CODE')),date=norm(docValue(h,'DOCUMENT_DATE')),hay=`${no} ${customer} ${code} ${date}`.toUpperCase();if(q&&!hay.includes(q))continue;const session=recordSession(h),found=findLatestDocument(type,no,session);rows.push({type,h,session,count:found?.items.length||0})}}rows.sort((a,b)=>String(docValue(b.h,'UPDATED_AT')||docValue(b.h,'DOCUMENT_DATE')).localeCompare(String(docValue(a.h,'UPDATED_AT')||docValue(a.h,'DOCUMENT_DATE'))));if(!rows.length){box.innerHTML='<div class="notice">找不到已保存的 Invoice／Consignment／Quotation。</div>';return}box.innerHTML='';for(const row of rows.slice(0,100)){const no=norm(docValue(row.h,'DOCUMENT_NO')),documentStatus=norm(docValue(row.h,'DOCUMENT_STATUS')).toUpperCase(),statusText=documentStatus==='CANCELLED'?'Cancelled':'Confirmed',card=document.createElement('div');card.className='recall-result';card.innerHTML=`<div><strong>${esc(no)}</strong><span>${row.type==='consignment'?'Consignment':row.type==='quotation'?'Quotation':'Invoice'} · Rev ${revisionNumber(row.h)} · ${esc(docValue(row.h,'DOCUMENT_DATE'))} · ${statusText}</span><small>${esc(docValue(row.h,'CUSTOMER_CODE'))} · ${esc(docValue(row.h,'CUSTOMER'))} · ${row.count} 件${norm(docValue(row.h,'EXHIBITION_NAME'))?` · ${esc(docValue(row.h,'EXHIBITION_NAME'))}`:''}</small></div><button type="button">重新開啟</button>`;$('button',card).onclick=()=>openRecallDocument(row.type,no,row.session);box.appendChild(card)}}
...
async function importGoldSilverFile(file){
  ...
  const latest=goldFileLatest();state.quote.currentLondonPmDate=latest.date;state.quote.currentLondonPm=latest.pm;state.quote.source='file';state.quote.historicalPm={};saveLatestLondonPmCache();saveHistoricalGoldCache();
  syncEffectivePrices({clearCurrentOverride:true});updateGoldQuoteUI();renderItems();schedulePreview();
...
loadInventoryHistory();loadDocumentStore();loadLatestLondonPmCache();loadHistoricalGoldCache();
...
async function importStockFile(f){
  ...
  if(!fullMap.size)throw new Error('找不到有效 LOTNO / ARTNO / PRICE');state.stockCatalog=fullMap;importDocumentStoreFromWorkbook(wb);importImageOverridesFromWorkbook(wb);rebuildInventoryMaps();for(const id of ['#exportJmsdataBtn','#exportImageOverridesBtn','#exportUpdatedPackageBtn']){const exportBtn=$(id);if(exportBtn)exportBtn.disabled=false}renderRecallResults();return fullMap.size;
}
...
$('#exhibitionPackageInput').onchange=async e=>{
  ...
  const root=(files[0].webkitRelativePath||'').split('/')[0]||'Exhibition Package';state.packageName=root;state.packageFiles=files;state.customPackageImages=new Map();state.packageImageDirtyFiles=new Set();
...
function renderItems(){
  ...
  displayItems().forEach(item=>{
    ...
    const manualFlag=$('.manual-price-flag',node);if(manualFlag)manualFlag.classList.toggle('hidden',!isManualPriceOverride(item));const thumbImg=getImg(item),thumb=$('.item-thumb',node);thumb.src=thumbImg?.url||placeholder(item.artNo);thumb.classList.toggle('grayscale-image',!!thumbImg?.grayscale);
...
function renderPreview(){
  ...
  const rows=items.map((x,index)=>{
    const selected=getImg(x),img=selected?.url||placeholder('No Image'),gray=selected?.grayscale?' grayscale-image':'';

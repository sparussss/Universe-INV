...
function updateGoldQuoteUI(message='',type=''){
  ...
  const m=goldMetrics(),fileLatest=goldFileLatest(),meta=$('#currentLondonPmMeta');$('#companyGoldBase').textContent=m?`USD ${m.base.toFixed(0)} / oz`:'—';$('#gold18PerGram').textContent=m?`USD ${m.g18.toFixed(3)}`:'—';$('#gold14PerGram').textContent=m?`USD ${m.g14.toFixed(3)}`:'—';if(meta)meta.textContent=fileLatest?`${state.quote.goldDataName} 最新：${fileLatest.date} · USD ${fileLatest.pm.toFixed(2)} / oz`:'GoldSilver.xlsx 未載入，可人手輸入日期及金價。';renderHistoricalGoldInputs();
